...
export class TranscriberDO {
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (fileSize > 0 && fileSize <= SMALL_FILE_LIMIT) {
        ...
        const base64Audio = this.toBase64(buffer);
        const aiResp = await this.env.AI.run("@cf/openai/whisper-large-v3-turbo", { audio: base64Audio });
        ...
        while (offset < fileSize) {
          ...
          for (let i = 0; i < MAX_PARALLEL && offset < fileSize; i++) {
            ...
            tasks.push(this.processChunk(url, inputUrl, offset, end));
            offset += chunkSize;
          }
      ...
      const merged = this.mergeResults(allChunks);
      const lastSegment =
      ...
      return this.json({ text: merged.text, segments: merged.segments, meta });
    } catch (err: any) {
      console.error("TranscriberDO error:", err);
  ...
  private json(obj: any, status = 200): Response {
  ...
  private async resolveUrl(inputUrl: string): Promise<string> {
    ...
    const resp = await fetch(
      ...
      { headers: { Authorization: `Bearer ${this.env.HARVEST_KEY}` } }
    );
  ...
  private mergeResults(chunks: Array<{ offset: number; result: any; chunkSize: number }>) {
    ...
    for (const s of globalSegments) {
      ...
      if (prev) {
        ...
        const similar = this.normalizeText(prev.text) === this.normalizeText(s.text);
  ...
  private async processChunk(
    ...
    if (resp.status === 403 || resp.status === 401) {
      url = await this.resolveUrl(inputUrl);
    ...
    const aiResp = await this.env.AI.run("@cf/openai/whisper-large-v3-turbo", {
      audio: base64Audio,
    });
